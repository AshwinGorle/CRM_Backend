...
class BusinessDevelopmentController {
    static createBusinessDevelopment = catchAsyncError(async (req, res, next) => {
        ...
        if (!entryDate || !enteredBy) {
            return res.status(400).json({ status: 'failed', message: 'Required fields must be filled' });
        }
        ...
        res.status(201).json({ status: 'success', message: 'Business Development created successfully', data: newBusinessDevelopment });
    });

    static getAllBusinessDevelopments = catchAsyncError(async (req, res, next) => {
    ...
    static getBusinessDevelopmentById = catchAsyncError(async (req, res, next) => {
        const { id } = req.params;
        ...
        if (!businessDevelopment) throw new ServerError("NotFound", "Business Development");
    ...
    static deleteBusinessDevelopment = catchAsyncError(async (req, res, next) => {
        ...
        const businessDevelopment = await BusinessDevelopmentModel.findByIdAndDelete(id);
