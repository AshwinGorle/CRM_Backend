...
class OpportunityController {
  static generateCustomID = (clientName) => {
    ...
    const alphanumericPart = Math.random().toString(36).substring(2, 6).toUpperCase();
  ...
  static createOpportunity = catchAsyncError(async (req, res, next) => {
    ...
    let customId = null;
    if(client){
        const fetchedClient = await ClientMasterModel.findById(client).select("name");
  ...
  static getAllOpportunities = catchAsyncError(async (req, res, next) => {
    ...
    const updatedOpportunities = opportunities.map((opportunity) => {
        ...
        const totalRevenue = plainOpportunity.revenue.reduce((accumulator, current) => {
            return (
                accumulator + current.Q1 + current.Q2 + current.Q3 + current.Q4
            );
        }, 0)
        const expectedSales = totalRevenue*(plainOpportunity.confidenceLevel / 100)
        return {
            ...plainOpportunity,
            totalRevenue,
            expectedSales
  ...
  static getOpportunityById = catchAsyncError(async (req, res, next) => {
    ...
    opportunity = { ...opportunity.toObject(), totalRevenue: allTimeRevenue, expectedSales : allTimeRevenue*(opportunity.confidenceLevel / 100) };
    res.status(200).json({
      status: "success",
  ...
  static updateOpportunity = catchAsyncError(async (req, res, next) => {
    ...
    if(updateData.client && !opportunity.customId){
        const fetchedClient = await ClientMasterModel.findById(updateData.client).select("name");
        const customOpportunityId = this.generateCustomID(fetchedClient.name);
